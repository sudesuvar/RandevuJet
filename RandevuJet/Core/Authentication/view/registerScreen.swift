        ...
        NavigationView {
            VStack(spacing: 30) {
                ...
                Button(action: {
                    handleRegister()
                    Task{
                        try await authViewModel.createUser(withEmail: email, password: password, fullName: nameSurname)
                    }
                }) {
                    Text("Kayıt Ol")
                        .font(.headline)
                ...
                .opacity(nameSurname.isEmpty || email.isEmpty || password.isEmpty ? 0.6 : 1.0)
...
// Preview
struct registerScreenPreview: PreviewProvider {
